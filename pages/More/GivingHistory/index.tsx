...
import {fetchAllGivingCall, fetchGivingCall, fetchLiveSubscriptionCall} from "../../../store/apiThunks/payment";
...
const GivingHistory: React.FC<NavigationProps> = ({navigation, route}) => {
    ...
    } = useQuery<GenericResponseType<FetchGivingPayloadType[]>>("fetchallgivings", async () =>
        await dispatch(fetchAllGivingCall({
    ...
    const handleSummaryClick = async (idx_: number) => {
        // dispatch(moreActions.setSelectedGivingTransaction(idx_));
        // navigation?.navigate(MoreRoutes.GiftSummary);

        await dispatch(fetchGivingCall({
            fetchGivingRequest: {
                reference: givingTransactions[idx_]?.reference || ""
            }
        }))
            .unwrap()
            .then((res) => {
